  ...
  store.subscribe((mutation) => {
    switch (mutation.type) {
      ...
      case types.COMPLETE_REQ_INTERCEPT:
        socket.emit(
          mutation.payload.INTERCEPT_REQUEST.intercept.id,
          mutation.payload.INTERCEPT_REQUEST
        )
        break
      case types.COMPLETE_RES_INTERCEPT:
        socket.emit(
          mutation.payload.INTERCEPT_RESPONSE.intercept.id,
          mutation.payload.INTERCEPT_RESPONSE
        )
        break
      default:
        break
